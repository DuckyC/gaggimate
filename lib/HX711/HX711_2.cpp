...
HX711_2::HX711_2() {
    _theHX711 = this;
    _hx711ReadTimer = timerBegin(0, 80, true);
    timerAttachInterrupt(_hx711ReadTimer, _onHX711ReadTimerInterrupt, false);
    timerAlarmWrite(_hx711ReadTimer, 1000, true); // 2 microseconds
    timerAlarmEnable(_hx711ReadTimer);
...
void HX711_2::get_value(long *readValues, const byte times) { read_average(readValues, times); }
...
long HX711_2::get_readCounter() { return (readStatus >> 6) & 0xFFFFFFUL; }

void HX711_2::power_down() {
    if (!(readStatus & 0x80000000UL)) {
        timerAlarmDisable(_hx711ReadTimer);
...
void HX711_2::processReadTimerInterrupt(void) {
    if (!(readStatus & 0x3FUL)) {
        if (digitalRead(DOUT) || digitalRead(DOUT2)) { // conversion not ready
            return;
        }
    }
...
void HX711_2::_onHX711ReadTimerInterrupt(void) { _theHX711->processReadTimerInterrupt(); }
